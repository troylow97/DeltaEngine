  ...
  for ( auto arch : em.archetypes )
    for ( auto chunk : arch->chunks )
      for ( size_t i = 0; i < chunk->header.index; i++ )
      {
        ...
        {
          for ( auto ref : rttr::type::get_types() )
          {
            if ( ref.get_metadata( "Key" ) == comp.type->hash )
            {
              writer.String( ref.get_name().to_string() );
              writer.StartObject();
              void *ptr = reinterpret_cast<void *>( reinterpret_cast<byte *>( chunk ) + comp.offset + i * comp.type->size );
              ref.get_method( "Serialize" ).invoke( rttr::instance(), writer, ptr );
              writer.EndObject();
            }
          }
        }
