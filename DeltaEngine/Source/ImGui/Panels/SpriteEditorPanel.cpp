...
* \brief  The file contains implementation of SpriteEditorPanel.
      ...
      if (!m_editor.selectedFile.empty())
      {
        ...
        if (texture && ImGui::IsMouseDragging(0))
        {
          ...
          if (infoID >= 0 && infoID < info.size())
          {
            ...
            info[infoID].size.x = Math::Clamp(info[infoID].size.x, 1.f,
                                              1.f * texture->GetWidth() - info[infoID].offset.x);
            info[infoID].size.y = Math::Clamp(info[infoID].size.y, 1.f,
                                              1.f * texture->GetHeight() - info[infoID].offset.y);
          ...
          if (texture)
          {
            ...
            if (ImGui::BeginMenu("Texture Properties"))
            {
              ...
              if (ImGui::Button("Save"))
                texture->UpdateWrapMode(static_cast<int>(texture->wrapMode));
              ImGui::EndMenu();
            }
          ...
          if (ImGui::BeginChild("Texture Editing"), ImGuiWindowFlags_HorizontalScrollbar | ImGuiWindowFlags_NoMove)
          {
            ...
            if (selectedInfoID >= 0 && selectedInfoID < info.size())
            {
              ...
              if (ImGui::BeginChild("Sprite Properties", ImVec2(300, 160), true))
              {
                ...
                info[selectedInfoID].offset.x = Math::Clamp(info[selectedInfoID].offset.x, 0.f,
                                                            1.f * texture->GetWidth());
                info[selectedInfoID].offset.y = Math::Clamp(info[selectedInfoID].offset.y, 0.f,
                                                            1.f * texture->GetHeight());

                info[selectedInfoID].size.x = Math::Clamp(info[selectedInfoID].size.x, 1.f,
                                                          1.f * texture->GetWidth() - info[selectedInfoID].offset.x);
                info[selectedInfoID].size.y = Math::Clamp(info[selectedInfoID].size.y, 1.f,
                                                          1.f * texture->GetHeight() - info[selectedInfoID].offset.y);
