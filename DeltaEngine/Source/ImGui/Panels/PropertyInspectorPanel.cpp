...
* \brief  The file contains implementation of PropertyInspectorPanel.
    ...
    if (Editor::entity_selected)
    {
      ...
      if (auto result = em.GetEntityArchetype(index); result != nullptr)
      {
        ...
        for (auto& ref : result->components_desc->metalist)
        {
          ...
          ImGui::BeginGroup();
          if (ImGui::CollapsingHeader(instance.get_type().get_name().to_string().c_str(),
                                      ImGuiTreeNodeFlags_AllowItemOverlap))
          {
            ...
            for (auto property : instance.get_type().get_properties())
            {
              ...
              else if (prop_type.get_raw_type().is_enumeration())
              {
                ...
                ImGui::Combo(("##" + prop_name).c_str(), &current, c_ptr_vec.data(),
                             static_cast<int>(c_ptr_vec.size()));
