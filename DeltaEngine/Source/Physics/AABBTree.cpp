...
namespace DeltaEngine
{
	...
	void AABBTree::insertLeaf(unsigned leafNodeIndex)
	{
		...
		while (!_nodes[treeNodeIndex].isLeaf())
		{
			...
			float newParentNodeCost = 2.0f * combinedAabb.surface_area;
			float minimumPushDownCost = 2.0f * (combinedAabb.surface_area - treeNode.aabb.surface_area);
			...
			if (leftNode.isLeaf())
			{
				costLeft = leafNode.aabb.merge(leftNode.aabb).surface_area + minimumPushDownCost;
			}
			else
			{
				AABB newLeftAabb = leafNode.aabb.merge(leftNode.aabb);
				costLeft = (newLeftAabb.surface_area - leftNode.aabb.surface_area) + minimumPushDownCost;
			}
			if (rightNode.isLeaf())
			{
				costRight = leafNode.aabb.merge(rightNode.aabb).surface_area + minimumPushDownCost;
			}
			else
			{
				AABB newRightAabb = leafNode.aabb.merge(rightNode.aabb);
				costRight = (newRightAabb.surface_area - rightNode.aabb.surface_area) + minimumPushDownCost;
			}
