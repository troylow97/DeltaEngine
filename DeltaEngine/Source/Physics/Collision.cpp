...
namespace DeltaEngine
{
  ...
  bool CollisionIntersection_RectRect_Static(const Vector2 Center1, const Vector2 Size1, const Vector2 Center2,
                                             const Vector2 Size2)
  {
  ...
  //need to test
  bool CollisionIntersection_RectRay_Static(const Vector2 Center1, const Vector2 Size1, const Vector2 Center2,
                                            const Vector2 Size2)
  {
  ...
  bool CollisionIntersection_RectCircle(Collider& col1, RigidBody& r1, Collider& col2, RigidBody& r2, Manifold& m)
  {
    ...
    {
      float DistanceBetweenCircleTipAndRectCenter = std::sqrt(
        ((CircleTip.x - rect_center.x) * (CircleTip.x - rect_center.x)) +
  ...
  bool CollisionIntersection_RectLine_Static(const Vector2 Center1, const Vector2 Size1, const Vector2 Center2,
                                             const Vector2 Size2)
  {
    AABB aabb{Center1, Size1};
    LineSegment line{Center2, Size2};

    if (((line.m_pt0.x > aabb.min.x && line.m_pt0.x < aabb.max.x) || (line.m_pt1.x > aabb.min.x && line.m_pt1.x < aabb.
        max.x)) &&
      ((line.m_pt0.y > aabb.min.y && line.m_pt0.y < aabb.max.y) || (line.m_pt1.y > aabb.min.y && line.m_pt1.y < aabb.max
        .y)))
    {
      return true;
    }
  ...
  bool CollisionIntersection_CircleLineSegment_Static(const Vector2 Center1, const Vector2 Size1, const Vector2 Center2,
                                                      const Vector2 Size2)
  {
    Circle circle{Center1, Size1};
    LineSegment line{Center2, Size2};

    if ((line.m_pt0.x - circle.m_center.x) * (line.m_pt0.x - circle.m_center.x) + (line.m_pt0.y - circle.m_center.y) * (
        line.m_pt0.y - circle.m_center.y) <=
      (line.m_pt1.x - line.m_pt0.x) * (line.m_pt1.x - line.m_pt0.x) + (line.m_pt1.y - line.m_pt0.y) + (line.m_pt1.y -
        line.m_pt0.y))
    {
      return true;
    }
    if ((line.m_pt1.x - circle.m_center.x) * (line.m_pt1.x - circle.m_center.x) + (line.m_pt1.y - circle.m_center.y) * (
        line.m_pt1.y - circle.m_center.y) <=
      (line.m_pt1.x - line.m_pt0.x) * (line.m_pt1.x - line.m_pt0.x) + (line.m_pt1.y - line.m_pt0.y) + (line.m_pt1.y -
        line.m_pt0.y))
    {
      return true;
  ...
  bool CollisionIntersection_CircleCircle_Static(const Vector2 Center1, const Vector2 Size1, const Vector2 Center2,
                                                 const Vector2 Size2, Manifold& m)
  {
  ...
  bool CollisionIntersection_RayLine_Static(const Vector2 Center1, const Vector2 Size1, const Vector2 Center2,
                                            const Vector2 Size2)
  {
  ...
  bool CollisionIntersection_LineLine_Static(const Vector2 Center1, const Vector2 Size1, const Vector2 Center2,
                                             const Vector2 Size2)
  {
    LineSegment line{Center1, Size1};
    LineSegment line2{Center2, Size2};
    float denominator = ((line.m_pt1.x - line.m_pt0.x) * (line2.m_pt1.y - line2.m_pt0.y)) - ((line.m_pt1.y - line.m_pt0.
      y) * (line2.m_pt1.x - line2.m_pt0.x));
    float numerator1 = ((line.m_pt0.y - line2.m_pt0.y) * (line2.m_pt1.x - line2.m_pt0.x)) - ((line.m_pt0.x - line2.m_pt0
      .x) * (line2.m_pt1.y - line2.m_pt0.y));
    float numerator2 = ((line.m_pt0.y - line2.m_pt0.y) * (line.m_pt1.x - line.m_pt0.x)) - ((line.m_pt0.x - line2.m_pt0.x
    ) * (line.m_pt1.y - line.m_pt0.y));
  ...
  {
    if ((Center2.x - Center1.x) * (Center2.x - Center1.x) + (Center2.y - Center1.y) * (Center2.y - Center1.y) <= (Size1.
      x * Size1.x))
    {
      return true;
    }
