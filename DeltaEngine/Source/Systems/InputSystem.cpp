...
{
  env.pECS->GetWorld().GetEntityManager().ForEach( [&]( EntityID id1, Player &p1, Input &i1, RigidBody &r1, State &s1, Attack &a1, Image &im1, Collider &c1, Animator &anim, AudioSource &audio )
  {
...
{
  env.pECS->GetWorld().GetEntityManager().ForEach( [&]( EntityID id1, Player &p1, RigidBody &r1, State &s1, Attack &a1, AudioSource &audio )
  {
    p1.IsRunning = false;
