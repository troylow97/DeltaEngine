...
namespace DeltaEngine
{
  ...
  void Material::SetUniform1i(const std::string& name, int i0)
  {
    GetEnv().pManager->Get<Shader>(m_ShaderKey)->SetUniform1i(name, i0);
  }
  void Material::SetUniform1f(const std::string& name, float f0)
  {
    GetEnv().pManager->Get<Shader>(m_ShaderKey)->SetUniform1f(name, f0);
  }
  void Material::SetUniformMatrix4f(const std::string& name, const Matrix4x4& matrix)
  {
    GetEnv().pManager->Get<Shader>(m_ShaderKey)->SetUniformMatrix4f(name, matrix);
  }
  void Material::SetUniformColor4f(const std::string& name, Color& c)
  {
    GetEnv().pManager->Get<Shader>(m_ShaderKey)->SetUniformColor4f(name, c);
  }
  void Material::SetUniformVector3f(const std::string& name, Vector3& v)
  {
    GetEnv().pManager->Get<Shader>(m_ShaderKey)->SetUniformVector3f(name, v);
  }
  void Material::SetUniformVector4f(const std::string& name, Vector4& v)
  {
    GetEnv().pManager->Get<Shader>(m_ShaderKey)->SetUniformVector4f(name, v);
  }
  void Material::SetUniformVector4f(const std::string& name, float v0, float v1, float v2, float v3)
  {
